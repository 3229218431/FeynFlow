...
📌 **一句话本质**：旋度就是看矢量场在一点上"打转的厉害程度和转轴方向"——像把一个小风车放进场里，它转得越快，旋度越大，转轴就是旋度方向。

🏷️ **重要度**：🔴 | 考试：★★★ | 题型：计

🔧 **工程/实际场景**：电机电磁设计——计算电机转子在磁场中受到的转矩时，通过安培定律的旋度形式 ∇×H = J 确定导体周围的磁场分布。如果旋度方向（右手定则）判反，扭矩方向就反了，电机会反转——一台设计前进的电动汽车会变成倒退。
...
## ✂️ 可跳过
- 旋度在矩形框上的泰勒展开推导过程考试不要求默写，只需记住行列式表达式 (1-6-7)
- 例1-6-2（旋度定理三维推广）的证明为加深内容，首次学习可跳过
- vega-lite 和 wavedrom 图表帮助直觉理解，无需背诵数据
